...
def run_evaluation() -> dict[str, Any]:
    ...
    documents = {item["id"]: item["text"] for item in retrieval["documents"]}
    retrieval_results = []
    for query in retrieval["queries"]:
        ...
        ranked = sorted(
            documents,
            key=lambda doc_id: (
                -len(query_tokens & _tokens(documents[doc_id])),
                doc_id,
            ),
        )
        actual = ranked[0]
        retrieval_results.append(
            {
                ...
                "top3_passed": query["expected"] in ranked[:3],
            }
        )
    ...
    retrieval_passed = sum(item["passed"] for item in retrieval_results)
    retrieval_top3_passed = sum(item["top3_passed"] for item in retrieval_results)
    ...
    return {
        ...
        "is_test_data": True,
        "claim_boundary": "合成评测只验证当前示例规则和测试检索器，不代表真实诊断能力",
        "diagnosis": {
        ...
        "retrieval": {
            ...
            "top1_accuracy": retrieval_passed / len(retrieval_results),
            "top3_accuracy": retrieval_top3_passed / len(retrieval_results),
            "threshold": 0.9,
        ...
        "passed": (
            ...
            and retrieval_passed / len(retrieval_results) >= 0.9
            and retrieval_top3_passed == len(retrieval_results)
