...
Phase 1 已落地 `frontend/`、`backend/`、根配置和容器骨架。设备端、模拟器、知识库与业务测试目录将在对应阶段创建，避免空目录冒充实现。
...
1. 当前目录不是 Git 仓库，无法依靠提交历史进行增量比较和回滚。
2. Docker Desktop 4.82.0 已安装，Compose 三服务运行验收通过；PlatformIO 仍不可用，将在设备阶段处理。
3. 本机使用 Python 3.9.6 虚拟环境完成后端测试，容器使用 Python 3.12 且运行验收通过；后续仍应持续验证二者行为一致。
4. Node.js v26.3.0 已通过本地 lint、Vitest、类型检查、构建和 Playwright，但生产容器固定使用 Node 22，降低部署兼容风险。
5. AI Provider、ESP32 型号、接线、账号和知识来源尚未确定，必须继续使用 `TODO[待补充]`。
6. 背景 Word 的旧技术草案与固定方案有差异，已在 `PROJECT_CONTEXT.md` 中明确裁决，后续不得同时保留两套实现。
