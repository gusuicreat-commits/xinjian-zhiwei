...
Phase 3 已落地 `frontend/`、`backend/`、数据库模型、设备认证、采集 API 和可配置测试模拟器。真实设备端与知识库将在对应阶段创建，避免空目录冒充实现。
...
1. Git 已初始化且存在 Phase 1 基线；Phase 2 变更完成验收但尚未提交。
2. Docker Desktop 4.82.0 已安装，Compose 三服务运行验收通过；PlatformIO 仍不可用，将在设备阶段处理。
3. 本机 Python 3.9.6 与容器 Python 3.12 均已验证 Phase 2；后续仍应持续验证二者行为一致。
