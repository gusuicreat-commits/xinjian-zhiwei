...
日期：2026-09-12。范围：第二阶段发现的三个软件问题及其重试、并发、恢复边界。25 个 PostgreSQL 完整流程场景、42 项反馈可靠性测试、后端全量 330 项及迁移验收通过。

## 项目负责人可读摘要

| 问题 | 本轮结论 |
| --- | --- |
| 这次发现了什么问题？ | 上轮发现三件事：建议可能借用不相关的真实证据；一次反馈重试会被当成两次操作；另一学生的实验会话可以改变原学生的诊断。整改中进一步补上了“系统已处理，但成功回执丢失”的恢复边界。 |
| 会造成什么现实影响？ | 学生可能被不相关的依据误导；网络不稳定时重复推进、重复调用 AI；诊断可能被错误归属的反馈结束。仅让正常路径跑通，不能排除这些问题。 |
| 现在能不能解决？ | 三项均为软件问题。本轮已修正证据关联、请求重试身份和写入前归属校验，并增加故意制造错误、丢失回执及并发提交的回归；最终后端门禁结果见下表。 |
| 如果不能，需要硬件还是教师确认？ | 修复上述软件问题不依赖硬件或教师。但失败日志能否区分真实根因、LED 是否发光、合理阈值需要硬件；教学步骤、验收标准和真实案例需要教师。本轮不升级这些事实状态。 |
| 下一步最应该做什么？ | 先确认本轮隔离验证结果，再按迁移与前后端兼容要求安排部署；真实硬件继续采用正常—单因素故障—恢复对照，不能用本轮软件测试代替。 |
...
- 提交前将 `request_id` 与 action 存入当前标签页的 `sessionStorage`，按设备、实验会话、诊断隔离；该重试记录不保存设备 token。
...
- `sessionStorage` 只承诺当前标签页的刷新恢复，**关闭标签页后不承诺保存重试键**。当前没有跨标签页或跨设备的未决反馈管理页面。
...
当前完整流程集合为 **25 个合成场景，全部通过**。原始失败记录继续保留在 [第二阶段报告](workflow-evaluation-phase2.md)，新结果保存在 [整改后 PostgreSQL 评测记录](../output/workflow-evaluation/remediation-postgres.json)。报告仅含合成输入和 Mock 输出，不能作为真实硬件准确率。
...
最终流程报告的代码与夹具 hash 已逐项核对当前工作区。AST 比较确认当前图与基线的节点、边及条件边声明一致。历史 `phase2-postgres.json` 仍保留 13 passed / 3 failed，不用整改后的结果覆盖失败基线。
