...
九个 V2 核心节点分别负责构建状态、规则判定、故障树候选生成、推理前知识供给、受约束 AI 原因排序、推理后知识校验、教学语言解释、反馈接收和升级决策。`teacher_review`、`persist_result`、`reject_result` 是审核与持久化基础设施节点。
...
error_type / evidence / possible_causes / knowledge_context / knowledge_supply
historical_failures / hint_level / student_feedback
attempt_count / missing_evidence / next_verification_action
evidence_conflict / knowledge_validation / need_teacher_help / diagnosis_status
```
...
- `knowledge_context` 只匹配已批准、根因已确认、事实锁定且质量检查通过的 `KnowledgeCase`，并在推理前供给模型。
- `ai_reasoning` 只能使用故障树已有 `cause_id`，引用证据 ID 并输出离散支持等级；越界即回退。
- `knowledge_validation` 在推理后以确定性代码检查规范、证据 ID、候选集、规则结果和允许动作；失败时请求教师介入。
