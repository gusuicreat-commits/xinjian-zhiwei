...
| P4 | 五类角色、会话、权限分离、班级/设备资源范围 | `backend/app/services/rbac.py`、`backend/app/api/v1/routes/auth.py` | `20260727_0011` + `20260730_0015` | `/api/v1/auth/session`、`/me`、`/classes`、`/devices` | `backend/tests/test_classroom_auth.py`、`test_knowledge_api.py` |
...
| P6 | 文件解析、来源和版本、块编辑/拆分/合并、双角色审核、正式检索隔离 | `backend/app/services/knowledge.py`、`knowledge_files.py`、`routes/knowledge.py` | 沿用 `20260720_0007`，`0015` 收敛角色和状态 | `/api/v1/knowledge/*` | `backend/tests/test_knowledge_api.py`、`test_knowledge_workspace.py` |
...
| P9 | 统一版本、锁文件、CI、迁移门禁、敏感信息扫描 | `VERSION`、`.github/workflows/ci.yml`、`scripts/verify.sh`、`security_scan.sh` | `20260730_0015` | `scripts/verify.sh`、`scripts/check_version.py` | CI YAML 解析、全套自动化与空库迁移 |
...
- 合成评测的诊断 `30/30` 与混合检索 `54/54` 只表示固定测试集符合当前示例规则和标注预期。
