...
返回设备在线/离线/未上报与异常数量、互斥的设备状态图数据、全部诊断记录的错误排行与七日趋势、最新异常设备、最近 80 条设备日志及 Level 4 介入队列。学生、班级、实验任务和知识案例尚未建模，对应结构明确返回 `configured=false`，实验完成率返回 `null`。
...
当前提供设备凭据保护的临时学生 API，以及审阅令牌保护的临时教师聚合 API；不提供正式账号、班级、实验任务或知识库业务 API，也不提供公开设备注册接口。
