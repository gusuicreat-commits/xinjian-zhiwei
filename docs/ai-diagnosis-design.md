...
- 兼容知识目录目前只有 DHT11、LED、按键、光敏传感器和超声波五类结构化案例，规模小且字段稳定；完整包化示例当前只有 DHT11 和 LED。
...
- `knowledge_context`：在 AI 推理前供给已审核的实验定义、正常条件、故障映射和教师确认案例。实验包案例必须同时匹配当前错误类型。
...
- `cause_id` 必须来自故障树；`used_evidence_ids` 必须来自本次诊断实际落库的 `diagnosis_evidence.id` UUID；`error_type` 必须与规则结果一致。运行时不得临时生成伪证据 ID。
...
- 第一阶段没有 RAG 分支、Embedding 调用或向量检索。历史迁移仍要求数据库镜像提供 `vector` 扩展，但诊断运行时不使用它。

当前边界还包括：未解决反馈不会自动重新采集设备数据；推理后校验覆盖证据/原因身份、集合关系、动作白名单和基本规范一致性，并不等价于完整电气安全证明。具体限制与变更门禁见 [开发准则](development-guidelines.md)。
...
## 12. 已实施的输出边界补强（2026-09-12）

第一阶段没有改变 LangGraph 节点/边、AI 候选空间或实验包发布机制。现有校验新增 high 的有效引用要求和输入冲突保护；无关联证据的确定性降级返回 unknown，允许动作保持可用。

最终解释 `steps` 逐字选择当前允许动作；`summary` 与 `limitations` 改为后端生成，减少自由文字升级事实的机会。当前输出契约存入现有审计 JSON；缓存纳入实际 Prompt 哈希并重新校验，旧审计缺契约时不作为新建议展示。推理自由文本、教师编辑和完整因果支持仍不在这次模板保护的证明范围内。

测试和兼容细节见 [评测要求对应表](evaluation-requirements.md)。

## 13. PDF 反例整改（2026-09-15）

输出契约升级为 `explanation-boundary-v2`：摘要列出本次规则异常，并反映 unknown 状态；限制说明保留证据冲突以及推理提出的具体补充证据需求。后者以“推理提出的待核验项（未确认）”引用，不能自动视为已确认缺失或硬件事实。解释模型单独补写的“已确认”限制仍不直接采纳，动作白名单不放宽。

输入继续经过原脱敏流程，原推理状态不删除；响应、持久化解释和回放使用相同契约。旧 v1 审计不改写，不符合当前契约时沿用确定性降级；没有新增迁移。模型原始自由文字与已生成解释仍不是同一审计材料，本轮没有新增原文存储。

关键词出现只作审阅线索，不是语义判定。Rubric 只允许逐条“符合/不符合”；本轮建立条目和未执行状态，不引入分数或模型裁判。自由推理 reason/summary 的完整语义仍待独立审阅，不能用模板保护或代码通过替代。
