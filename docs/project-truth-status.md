...
| 问题 | 本轮结论 |
| --- | --- |
| 这次发现了什么问题？ | 完整流程评测发现候选借用无关证据、反馈重试重复处理和会话归属缺口；本轮已修复，并补上回执丢失、并发和 Checkpoint 保存失败的恢复检查。 |
| 会造成什么现实影响？ | 真实 UUID 可能掩盖依据不相关；网络不稳定会重复推进或调用 AI；另一学生会话可能改变原诊断。 |
| 现在能不能解决？ | 上述软件问题已整改；25 个 PostgreSQL 场景、42 项可靠性检查及后端全量 330 项通过。代码仍不能证明真实接线、器件故障或灯是否发光。 |
| 如果不能，需要谁确认？ | 硬件负责人验证板卡、日志、测量来源和阈值；教师提供实验指导书、验收标准、操作边界及真实案例审核。 |
| 下一步最应该做什么？ | 按整改报告审阅迁移与接口兼容，再安排部署；硬件侧锁定器件和课程材料，完成正常—单因素故障—恢复对照。 |
...
| WF-ISSUE-02 反馈重试 | ✅ verified（软件已修） | 同诊断同键同载荷重放；改载荷 409；真实新尝试新键。前端保存当前标签页未决载荷，关闭标签页不承诺保留 |
...
| 完整流程与回归 | ✅ verified（合成） | PostgreSQL 25/25；后端 330 passed，无 skipped/xfail；前端 43 unit、4 学生 E2E，后者使用 Mock API |
