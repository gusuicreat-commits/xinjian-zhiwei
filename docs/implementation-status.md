...
## 17. 反馈找回与持续验收（2026-09-12）

第 16 节整改已先保存为本地提交 `3c7fcb1`，本轮在该基线上补齐使用与验收缺口。负责人五问摘要与最终门禁结果见 [整改报告](workflow-remediation.md)；此前各节测试数量为各轮历史记录。

- **服务端找回：** 新增只读 `GET /student/feedback-recovery`，核对设备、学生、会话和诊断原归属，返回至多 20 条未决反馈、是否还有更多及最近已应用回执；包含较早诊断。原 request_id/action/note 保留，历史 NULL 不补造。
- **学生页面：** 重登后展示服务端记录，明确点击才继续原反馈；查询失败或有未决记录时阻止新反馈。保留浏览器独有的未确认请求，不用服务器查询静默覆盖。请求从未落库且本地已丢失仍不能找回。
- **联调暴露的问题：** 有效会话没有历史诊断也显示启动入口；诊断工作流和解释请求增加会话/请求序号校验，旧响应不能覆盖新页面。
- **持续验收：** CI 和本地 verify 接入 25 场景 PostgreSQL 评测以及两项浏览器—FastAPI—PostgreSQL 联调；无测试 DSN 时明确失败，不回退应用连接。测试使用独立 schema、真实迁移、Mock AI，并断言退出清理。
