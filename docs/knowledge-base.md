...
- 保存文档、知识块、审核历史及向量记录。
- 教师审阅令牌持有者可批准或拒绝文档；批准状态同步到全部知识块。
...
- 没有确定 Embedding Provider、模型和向量维度。
...
| `knowledge_reviews` | 每次批准或拒绝的审核人引用、决定、备注和时间 |
...
5. 通过 `PATCH /api/v1/knowledge/documents/{document_id}/review` 审核文档。
6. 只有记录了 `authorization_scope` 的来源才能被批准。
7. 确认 Provider 后，由适配器生成向量并写入文档的 embeddings 接口。
8. 通过 `/api/v1/knowledge/search` 检索时只返回审核通过且模型维度匹配的知识块。
...
正式导入前必须至少确认：资料所有者、允许用途、可否保存全文、可否用于 Embedding/外部模型、版本、来源 URI、审核人和失效/替换方式。博客、论坛、AI 生成文本和无法追溯的文件不得直接批准为正式知识。
